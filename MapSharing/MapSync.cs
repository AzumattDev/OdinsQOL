...
namespace OdinQOL.MapSharing
{
    public class MapSync
    {
        ...
        public static void RPC_OdinQOLMapSync(long sender, ZPackage mapPkg)
        {
        ...
        public static void SendMapToServer()
        {
            ...
            if (exploredAreas.Count == 0)
            {
                ...
                ZRoutedRpc.instance.InvokeRoutedRPC(ZRoutedRpc.instance.GetServerPeerID(), "OdinQOLMapSync", pkg);
            }
        ...
        public static void LoadMapDataFromDisk()
        {
            ...
            //Load map data
            if (File.Exists(OdinQOLplugin.OdinQOL_DatadirectoryPath + $"{ZNet.instance.GetWorldName()}_mapSync.dat"))
                try
                {
                    string mapData = File.ReadAllText(Path.Combine(OdinQOLplugin.OdinQOL_DatadirectoryPath +
        ...
        public static void SaveMapDataToDisk()
        {
            ...
            {
                File.Delete(Path.Combine(OdinQOLplugin.OdinQOL_DatadirectoryPath +
                                         $"{ZNet.instance.GetWorldName()}_mapSync.dat"));
                File.WriteAllText(
                    Path.Combine(
                        OdinQOLplugin.OdinQOL_DatadirectoryPath + $"{ZNet.instance.GetWorldName()}_mapSync.dat"),
        ...
        private static List<ZPackage> ChunkMapData(List<MapRange> mapData, int chunkSize = 10000)
        {
            ...
            foreach (List<MapRange> thisChunk in chunkedData)
            {
                ...
                foreach (MapRange mapRange in thisChunk) pkg.WriteVPlusMapRange(mapRange);
