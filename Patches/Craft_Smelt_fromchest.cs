...
namespace VMP_Mod.Patches
{
    class Craft_Smelt_fromchest : BaseUnityPlugin
    {
        ...
        {
            if (isDebug.Value)
                Debug.Log((pref ? typeof(Craft_Smelt_fromchest).Namespace + " " : "") + str);
        }
        private void Awake()
        {
            context = this;
                    ...
                    if (instr.opcode == OpCodes.Callvirt)
                    {
                        ...
                        if (instrString.Contains("IsDLCInstalled"))
                        {
                            ...
                            if (targetRetIndex != -1)
                            {
                                ...
                                codes.Add(new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(Craft_Smelt_fromchest), "HaveRequiredItemCount")));
                                //Dbgl($">>> Inserting instruction to the end:: { codes[codes.Count-1].ToString()}");
