...
namespace VMP_Mod.Patches
{
    ...
    public static class RemoveWearNTear
    {
        ...
        {
            if (WearNTear_Patches.NoWeatherDam.Value)
            {
                __result = true;
            }
        }
    ...
    public static class WearNTear_ApplyDamage_Patch
    {
        private static bool Prefix(ref WearNTear __instance, ref float damage)
        {
            ...
            if (!(WearNTear_Patches.DisableStructintegrity.Value && __instance.m_piece && __instance.m_piece.IsPlacedByPlayer() && callingMethod != "UpdateWear"))
                return true;
    ...
    public static class RemoveStructualIntegrity
    {
        private static bool Prefix(ref WearNTear __instance, out float maxSupport, out float minSupport, out float horizontalLoss, out float verticalLoss)
        {
            ...
            WearNTear_Patches.StructuralIntegritywood.Value = (WearNTear_Patches.StructuralIntegritywood.Value >= 100 ? 100 : WearNTear_Patches.StructuralIntegritywood.Value);
            WearNTear_Patches.StructuralIntegritystone.Value = (WearNTear_Patches.StructuralIntegritywood.Value >= 100 ? 100 : WearNTear_Patches.StructuralIntegritystone.Value);
            WearNTear_Patches.StructuralIntegrityiron.Value = (WearNTear_Patches.StructuralIntegritywood.Value >= 100 ? 100 : WearNTear_Patches.StructuralIntegrityiron.Value);
            WearNTear_Patches.StructuralIntegrityhardWood.Value = (WearNTear_Patches.StructuralIntegritywood.Value >= 100 ? 100 : WearNTear_Patches.StructuralIntegrityhardWood.Value);
            ...
            switch (__instance.m_materialType)
            {
                ...
                case WearNTear.MaterialType.HardWood:
                    ...
                    horizontalLoss = 0.16666667f - ((0.16666667f / 100) * WearNTear_Patches.StructuralIntegrityhardWood.Value);
                    return false;
                default:
                    maxSupport = 0f;
                    minSupport = 0f;
